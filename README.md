...
| Признак | A. Монолит | B. Микросервисы по доменам |
|---|---|---|
| Количество деплойных единиц | 1 | ~6 |
| Владение данными | общая БД, схемы по доменам | у каждого сервиса своя БД |
| Стиль коммуникации | внутренние вызовы методов | sync REST + 1 async event |
| Согласованность | ACID внутри БД | per-service ACID + eventual между |
